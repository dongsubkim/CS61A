...
(lambda (x y) (lambda z (+ x (* x y))))
; expect (lambda (x y) (lambda z (+ x (* x y))))
...
f
; expect (lambda (x y z) (* x (+ y z)))
(define (g a b) (+ (* a 10) (/ b 2)))
; expect g
g
; expect (lambda (a b) (+ (* a 10) (/ b 2)))
...
;;; *****************
;; Our own test
(define (test a b) (define (HOF c d) (+ a b c d)) HOF)
; expect test
(test 1 2)
; expect (lambda (c d) (+ a b c d))
((test 1 2) 3 4)
; expect 10
(define awesome (lambda (x) (+ x x x x)))
; expect awesome
(define (pow_some x y) (+ (* x (awesome y)) (* y (awesome x))))
; expect pow_some
(pow_some 2 3)
; expect 48
...
(let ((x 3) (y x)) y)
; expect 10
(let ((x 3) (y (let ((x 4)) x))) y)
; expect 4
(let ((x 3) (y (let ((x 4)) 5))) x)
; expect 3
